		...
		Add-JobTracker -Name "Update" `
			...
			try
			{
				...
				if ($check -gt $current)
				{
					...
					Write-Output $UpdateDescription
				}
				else
				{
					[void](New-ItemProperty -Path 'HKCU:\Software\PK-Tools\Email Distribution\General' -Name Update -Value 0 -PropertyType String -Force)
			...
			if ($result) { Call-update_notification_psf -Title 'Update' -DescriptionLabel 'Description' -DescriptionText $result -UpdateLabel 'New version available!' }
		} -ArgumentList $ApplicationVersion
	...
	function Invoke-CheckForConfig
	{
		...
		{		
			...
			if ($XML.EmailDistribution.Description -eq 'Email Distribution Utility')
			{
				...
				if ($PreLoadedConfig.Email.FromAdress)
				...
				if ($xml.EmailDistribution.Configuration.ADLookupSkip.Enable)
				{
					...
					[PSCustomObject]@{ Time = $(Get-Date -f HH:mm:ss); Action = 'XMLOption'; Description = "ADLookupSkip is set to ""$ADLookupSkip""" } | Export-Csv -Path "$LogsBase\$((Get-Date).ToShortDateString())_log.csv" -NoTypeInformation -Encoding UTF8 -Append
				}
				if ($xml.EmailDistribution.Configuration.TextPatternSkip.Enable)
				{
					...
					[PSCustomObject]@{ Time = $(Get-Date -f HH:mm:ss); Action = 'XMLOption'; Description = "TextPatternSkip is set to ""$($xml.EmailDistribution.Configuration.TextPatternSkip.Enable)""" } | Export-Csv -Path "$LogsBase\$((Get-Date).ToShortDateString())_log.csv" -NoTypeInformation -Encoding UTF8 -Append
				}
			}
		}
	}
...
param($Title,$Data)
	...
	$contextmenustripRemoveItem.Size = '191, 26'
	#
...
function Call-Logs_psf
{
	...
	$buttonOpenLogsBase.Text = "Open logs location"
