		...
		{		
			...
			if ($XML.EmailDistribution.Description -eq 'Email Distribution Utility')
			{
				...
				if ($xml.EmailDistribution.Configuration.ADLookupSkip.Enable)
				{
					...
					[PSCustomObject]@{ Time = $(Get-Date -f HH:mm:ss); Action = 'ADLookupSkip'; Description = "ADLookupSkip is set to ""$ADLookupSkip""" } | Export-Csv -Path "$LogsBase\$((Get-Date).ToShortDateString())_log.csv" -NoTypeInformation -Encoding UTF8 -Append
				}
			}
		}
	}
